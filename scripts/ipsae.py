...
def main(pae_file_path,pdb_path,pae_cutoff,dist_cutoff, print_file):
    ...
    iptm_d0chn_min    = init_chainpairdict_zeros(unique_chains)
    ipsae_d0chn_min   = init_chainpairdict_zeros(unique_chains)
    ipsae_d0dom_min   = init_chainpairdict_zeros(unique_chains)
    ipsae_d0res_min   = init_chainpairdict_zeros(unique_chains)
    ...
    iptm_d0chn_minres    = init_chainpairdict_zeros(unique_chains)
    ipsae_d0chn_minres   = init_chainpairdict_zeros(unique_chains)
    ipsae_d0dom_minres   = init_chainpairdict_zeros(unique_chains)
    ipsae_d0res_minres   = init_chainpairdict_zeros(unique_chains)
    ...
    n0dom_min   = init_chainpairdict_zeros(unique_chains)
    n0res       = init_chainpairdict_zeros(unique_chains)
    n0res_max   = init_chainpairdict_zeros(unique_chains)
    n0res_min   = init_chainpairdict_zeros(unique_chains)
    ...
    d0dom_min   = init_chainpairdict_zeros(unique_chains)
    d0res       = init_chainpairdict_zeros(unique_chains)
    d0res_max   = init_chainpairdict_zeros(unique_chains)
    d0res_min   = init_chainpairdict_zeros(unique_chains)
    ...
    for chain1 in unique_chains:
        for chain2 in unique_chains:
            ...
            # pick min value for each chain pair for each iptm/ipsae type
            if chain1 > chain2:
                minvalue=min(iptm_d0chn_asym[chain1][chain2], iptm_d0chn_asym[chain2][chain1])
                if minvalue==iptm_d0chn_asym[chain1][chain2]: minres=iptm_d0chn_asymres[chain1][chain2]
                else: minres=iptm_d0chn_asymres[chain2][chain1]
                iptm_d0chn_min[chain1][chain2]=minvalue
                iptm_d0chn_minres[chain1][chain2]=minres
                iptm_d0chn_min[chain2][chain1]=minvalue
                iptm_d0chn_minres[chain2][chain1]=minres

                minvalue=min(ipsae_d0chn_asym[chain1][chain2], ipsae_d0chn_asym[chain2][chain1])
                if minvalue==ipsae_d0chn_asym[chain1][chain2]: minres=ipsae_d0chn_asymres[chain1][chain2]
                else: minres=ipsae_d0chn_asymres[chain2][chain1]
                ipsae_d0chn_min[chain1][chain2]=minvalue
                ipsae_d0chn_minres[chain1][chain2]=minres
                ipsae_d0chn_min[chain2][chain1]=minvalue
                ipsae_d0chn_minres[chain2][chain1]=minres

                minvalue=min(ipsae_d0dom_asym[chain1][chain2], ipsae_d0dom_asym[chain2][chain1])
                if minvalue==ipsae_d0dom_asym[chain1][chain2]:
                    minres=ipsae_d0dom_asymres[chain1][chain2]
                    minn0=n0dom[chain1][chain2]
                    mind0=d0dom[chain1][chain2]
                else:
                    minres=ipsae_d0dom_asymres[chain2][chain1]
                    minn0=n0dom[chain2][chain1]
                    mind0=d0dom[chain2][chain1]
                ipsae_d0dom_min[chain1][chain2]=minvalue
                ipsae_d0dom_minres[chain1][chain2]=minres
                ipsae_d0dom_min[chain2][chain1]=minvalue
                ipsae_d0dom_minres[chain2][chain1]=minres
                n0dom_min[chain1][chain2]=minn0
                n0dom_min[chain2][chain1]=minn0
                d0dom_min[chain1][chain2]=mind0
                d0dom_min[chain2][chain1]=mind0

                
                print(ipsae_d0res_asym[chain1][chain2], ipsae_d0res_asym[chain2][chain1])
                minvalue=min(ipsae_d0res_asym[chain1][chain2], ipsae_d0res_asym[chain2][chain1])
                if minvalue==ipsae_d0res_asym[chain1][chain2]:
                    minres=ipsae_d0res_asymres[chain1][chain2]
                    minn0=n0res[chain1][chain2]
                    mind0=d0res[chain1][chain2]
                else:
                    minres=ipsae_d0res_asymres[chain2][chain1]
                    minn0=n0res[chain2][chain1]
                    mind0=d0res[chain2][chain1]
                ipsae_d0res_min[chain1][chain2]=minvalue
                ipsae_d0res_minres[chain1][chain2]=minres
                ipsae_d0res_min[chain2][chain1]=minvalue
                ipsae_d0res_minres[chain2][chain1]=minres
                n0res_min[chain1][chain2]=minn0
                n0res_min[chain2][chain1]=minn0
                d0res_min[chain1][chain2]=mind0
                d0res_min[chain2][chain1]=mind0
    ...
    for pair in sorted(chainpairs):
        ...
        for pair in (pair1, pair2):
            ...
            if chain1 > chain2:
                ...
                iptm_af_value=iptm_af
                pDockQ2_value=min(pDockQ2[chain1][chain2], pDockQ2[chain2][chain1])
                if boltz1:
                    iptm_af_value=min(iptm_boltz1[chain1][chain2], iptm_boltz1[chain2][chain1])


                LIS_Score=(LIS[chain1][chain2]+LIS[chain2][chain1])/2.0
                outstring=f'{chain2}    {chain1}     {pae_string:3}  {dist_string:3}  {"min":5} ' + (
                    f'{ipsae_d0res_min[chain1][chain2]:8.6f}    '
                    f'{ipsae_d0chn_min[chain1][chain2]:8.6f}    '
                    f'{ipsae_d0dom_min[chain1][chain2]:8.6f}    '
                    f'{iptm_af_value:5.3f}    '
                    f'{iptm_d0chn_min[chain1][chain2]:8.6f}    '
                    f'{pDockQ[chain1][chain2]:8.4f}   '
                    f'{pDockQ2_value:8.4f}   '
                    f'{LIS_Score:8.4f}   '
                    f'{int(n0res_min[chain1][chain2]):5d}  '
                    f'{int(n0chn[chain1][chain2]):5d}  '
                    f'{int(n0dom_min[chain1][chain2]):5d}  '
                    f'{d0res_min[chain1][chain2]:6.2f}  '
                    f'{d0chn[chain1][chain2]:6.2f}  '
                    f'{d0dom_min[chain1][chain2]:6.2f}  '
                    f'{residues_1:5d}   '
                    f'{residues_2:5d}   '
                    f'{dist_residues_1:5d}   '
                    f'{dist_residues_2:5d}   '
...
if __name__ == "__main__":
    ...
    main(pae_file_path,pdb_path,pae_cutoff,dist_cutoff,print_file)