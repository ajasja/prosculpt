...
Any further keys you add to a filter's entry are passed to its `filter_backbone` function as arguments. See [plugins/README.md](plugins/README.md) for the full contract, what happens to rejected backbones, and the filters included with prosculpt.
...
## Filtering and post-filtering scoring
The pre-filtering scoring script (the one that runs on *every* model, described
above) can be overridden and given extra arguments from a job's config:
- `scoring_script` (optional): path to the scoring script to use instead of
  the default `scripts/scoring_script.py`. Resolved the same way a plugin's
  `filter_script` is: an absolute path is used as-is, a relative one is
  checked against the current working directory first, then against
  prosculpt's own installation directory.
- `scoring_script_arguments` (optional): a dict of extra CLI flags/values to
  pass to it, e.g. `--some_flag: some_value` - same shape as `pass_to_mpnn`/
  `pass_to_af` above.

A `filtering:` section can be added to a job's config to automatically select
"good" designs, without any manual post-processing:
- `filtering_parameters`: a dict of metric name -> `min`/`max` thresholds
  (either or both, e.g. `RMSD: {max: 2.0}`). Any metric not found in
  `output.csv` is skipped with a warning in the log, rather than erroring.
- `ranking_parameters` (optional): `metric`, `higher_better` (true/false), and
  `max_results` - if more models pass `filtering_parameters` than
  `max_results`, only the best `max_results` (by `metric`/`higher_better`)
  are kept.
- `post_filtering_scoring_script` (optional): path to a scoring script,
  following the same contract as `scoring_script.py` above, to run
  against the *filtered* subset only - useful for a more expensive scoring
  pass (e.g. Rosetta interface analysis) that isn't worth running on every
  model. See `scripts/post_filtering_scoring_script.py` for an example (ddG/
  ShapeComplementarity per chain-chain interface). When set,
  `slurm_runner.py` automatically submits this as a second, dependent SLURM
  job alongside the main one (see `filtering_slurm:` for its own slurm
  options, and `config/installation.yaml`'s `filtering_slurm:` for
  cluster-wide defaults).
- `post_filtering_scoring_script_arguments` (optional): same idea as
  `scoring_script_arguments` above, but passed to `post_filtering_scoring_script`
  instead.

The generated `ps2slurm_*.txt` command file(s) `slurm_runner.py` submits to
`sbatch` are written directly into the job's own output directory (not the
repo root), so they stay next to the run they belong to instead of piling up
wherever `slurm_runner.py` happened to be invoked from.
...
- `output_dir`: output directory. Along the pipeline, each module will create its own subdirectory. The AF2 models are in the end renamed and copied to a subdirectory called `output_pdbs` (and, after filtering, `filtered_pdbs`)
...
A note on outputs: when running on a cluster for each task a separate `output.csv` (and `filtered_output.csv`) will be created. Run `merge_csv.py` along with the argument `--output_dir` to merge all csv files into one file.
...
The `dashboard/` directory holds an optional web interface for prosculpt, useful if you'd rather not hand-write YAML and tail log files:

- **Run job** - build a job's config through a form (with the raw YAML kept in sync alongside it, editable either way), upload an input PDB or filter script, then submit it. Runs `slurm_runner.py` either on the machine the dashboard runs on, or over SSH to a cluster.
- **Track job** - point it at a job's output directory to watch the run: current stage, progress and ETA, per-step timings, crash/cancellation banners, plus the RFdiffusion backbones, MPNN sequences and predicted models in a 3D viewer, and the metrics from `output.csv`/`filtered_output.csv` as a sortable, filterable table with a zip export. Several jobs can be tracked at once for a combined view across all of them.

It reads everything straight off disk, so it needs filesystem access to a job's output directory - typically by running on the cluster itself. To start it:
...
See [dashboard/README.md](dashboard/README.md) for configuring run targets, keeping it running under `screen` (or Windows Task Scheduler), the optional password gate, and the rest.
