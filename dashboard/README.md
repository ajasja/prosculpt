...
- **Submit an existing project directory** - point at a directory you've already prepared by hand; submitted as-is (copied over first for an `ssh` target), nothing rewritten behind your back. The chosen config file is loaded into an editor underneath, with **Save** / **Save as…** - the job always runs from what's on disk, so unsaved edits are warned about at submit time. A **Re-run final operations and post-filtering scoring** checkbox there sets `only_run_analysis` in that config; a run submitted with it reuses the existing `output_dir` rather than picking a fresh one, since there'd be nothing to analyse in a new directory. One project directory can be resubmitted more than once - if its `output_dir` is still there from a previous run, a fresh `_2`, `_3`, ... is picked automatically.
