...
from flask import Flask, jsonify, request, send_file, abort, redirect, render_template, session, url_for
...
# Only needed to sign the login session cookie (see login()/_require_auth()
# below) - random per process, so a dashboard restart simply logs everyone
# out again rather than needing its own persisted secret anywhere.
app.secret_key = os.urandom(24)


@app.before_request
def _require_auth():
    """Optional login gate - see `auth:` in dashboard_config.yaml.example.
    Disabled (the default with no config file at all) lets every request
    through exactly as before this existed. Static assets and the login
    route itself are always exempt, or the login page itself couldn't load
    its own CSS/JS, or a POST there would just redirect to itself forever."""
    auth = RT.get_auth_config()
    if not auth["enabled"]:
        return None
...
def login():
    ...
    if request.method == "POST":
        if request.form.get("password") == auth["password"]:
            session["authed"] = True
...
def api_trb():
    ...
    # tab's path_rfdiff, read verbatim from output.csv (see
...
def _load_merged_final_csv(job_dir: str):
    """Merges every task's own output.csv into one table - union of
    ...
    own tasks. None if no task has an output.csv yet."""
...
def api_final_pdb():
    ...
    task_dir = _get_task_dir(job_dir, request.args.get("task"))
    name = request.args.get("name")
    final_dir = P.resolve_final_pdbs_dir(task_dir)  # handles old "final_pdbs" dirs too
    path = _safe_join(final_dir, name)
    if not os.path.isfile(path):
        abort(404, description="pdb not found")
    return send_file(path, mimetype="chemical/x-pdb")
...
def api_final_csv():
    ...
    if result is None:
        abort(404, description="output.csv not found")
    return jsonify(result)


@app.route("/api/export_filtered", methods=["POST"])
def api_export_filtered():
    """Zips up the currently-filtered output.csv rows plus the
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        # filename can exist in more than one task's own output_pdbs/.
        if "model_path" in columns and "task" in columns:
            ...
            # same as output.csv itself (see load_final_csv()'s
...
# config), mirroring the /api/results routes above but backed by
# P.filtered_results_summary()/P.load_filtered_csv() instead.
# ---------------------------------------------------------------------------


def _load_merged_filtered_csv(job_dir: str):
    """Mirrors _load_merged_final_csv(), but merges every task's own
    filtered_output.csv instead of its output.csv."""
    all_columns: list[str] = []
    per_task = []
    for task_num, task_dir in P.discover_tasks(job_dir):
        result = P.load_filtered_csv(task_dir)
        if result is None or not result["columns"]:
            continue
        columns, rows = result["columns"], result["rows"]
        for c in columns:
...
@app.route("/api/filtering_results")
def api_filtering_results():
    """Mirrors api_results(), but for the filtered subset."""
    job_dir = _require_job_dir()
    final_pdbs = []
    any_csv = False
    all_finished = True
    ...
    for task_num, task_dir in P.discover_tasks(job_dir):
        any_task = True
        summary = P.filtered_results_summary(task_dir)
        for name in summary["final_pdbs"]:
            final_pdbs.append({"name": name, "task_num": task_num})
        any_csv = any_csv or summary["csv_exists"]
        all_finished = all_finished and summary["finished"]
...
@app.route("/api/filtered_pdb")
def api_filtered_pdb():
    """Mirrors api_final_pdb(), but reads from a task's filtered_pdbs/."""
    job_dir = _require_job_dir()
    task_dir = _get_task_dir(job_dir, request.args.get("task"))
    name = request.args.get("name")
    path = _safe_join(task_dir, "filtered_pdbs", name)
    if not os.path.isfile(path):
        abort(404, description="pdb not found")
    return send_file(path, mimetype="chemical/x-pdb")


@app.route("/api/filtering_csv")
def api_filtering_csv():
    """Mirrors api_final_csv(), but for filtered_output.csv."""
    job_dir = _require_job_dir()
    result = _load_merged_filtered_csv(job_dir)
    if result is None:
        abort(404, description="filtered_output.csv not found")
    return jsonify(result)


@app.route("/api/export_filtering_results", methods=["POST"])
def api_export_filtering_results():
    """Mirrors api_export_filtered(), but zips up filtered_output.csv rows
    plus each row's (already-filtered) model_path pdb."""
    job_dir = request.args.get("job_dir")
    data = request.get_json(silent=True) or {}
    job_dir = data.get("job_dir") or job_dir
    row_ids = data.get("row_ids")
    ...
    return send_file(
        ...
        mimetype="application/zip",
        as_attachment=True,
        download_name="prosculpt_filtered_results_export.zip",
    )


@app.route("/api/export_filtering_results_multi", methods=["POST"])
def api_export_filtering_results_multi():
    """Mirrors api_export_filtered_multi(), but across every tracked job's
    filtered_output.csv instead of output.csv."""
    data = request.get_json(silent=True) or {}
    jobs = data.get("jobs")
    if not jobs:
        abort(400, description="Missing jobs")
    ...
    return send_file(
        ...
        mimetype="application/zip",
        as_attachment=True,
        download_name="prosculpt_all_jobs_filtered_results_export.zip",
    )
...
def api_browse():
    ...
    # Directories only by default (a tracked job is pointed at by directory,
    # not by picking one specific file) - ?include_files=1 also lists files,
    # for the Run job "existing project" file pickers (see openBrowsePicker()
    # in app.js).
...
if __name__ == "__main__":
    # PORT env var wins if set (a per-invocation override); otherwise
    # `defaults: {port: ...}` in dashboard_config.yaml; otherwise 5000.
    port = int(os.environ.get("PORT") or RT.get_defaults().get("port") or 5000)
    ...
    # mount under a glob(), a big output.csv, a wedged console write
