...
if not args[0].dry_run:
    ...
    os.system(f"export GROUP_SIZE=1; sbatch --partition=intel --ntasks=1 --cpus-per-task=1 --output slurm-%A_%a_{task_name}.out --error slurm-%A_%a_{task_name}.out -J {task_name}  -a 1-{n} {slurm_runner_path}/wrapper_slurm_array_job_group.sh {out_command_file}")
